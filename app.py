...
# ============================================
# CONFIG - আপনার ডোমেইন এখানে সেট করুন
# ============================================
MAIN_DOMAIN = "localhost:5000"  # আপনার ডোমেইন বা আইপি এখানে দিন
ADMIN_SECRET = "admin_panel_2026"  # অ্যাডমিন প্যানেল সিক্রেট পাথ
...
# ============================================
# VIP USER MANAGEMENT (NEW)
# ============================================

def get_vip_users():
    users = load_json_file(VIP_USERS_FILE)
    if not users:
        users = {
            "sakil2026": {
        ...
        save_json_file(VIP_USERS_FILE, users)
    return users

def save_vip_users(users):
    return save_json_file(VIP_USERS_FILE, users)

def get_vip_settings():
    settings = load_json_file("vip_settings.json")
    if not settings:
        settings = {
            "main_domain": MAIN_DOMAIN,
            "packages": {
                "basic": {"price": 499, "days": 30, "daily_limit": 50},
                "premium": {"price": 999, "days": 60, "daily_limit": 100},
                "enterprise": {"price": 1999, "days": 90, "daily_limit": 500}
            }
        }
        save_json_file("vip_settings.json", settings)
    return settings
...
def get_user_expiry(username):
    ...
    if username not in users:
        return None
    return users[username].get("expiry", "")

def get_remaining_seconds_for_user(username):
    expiry_str = get_user_expiry(username)
    if not expiry_str:
        return 0
    try:
...
def is_user_expired(username):
    return get_remaining_seconds_for_user(username) <= 0

def verify_vip_user(username, password):
    users = get_vip_users()
    if username not in users:
        return False
...
def get_user_url(username):
    ...
    return users[username].get("custom_url", username.lower())

def get_user_package(username):
    users = get_vip_users()
    if username not in users:
        return "basic"
    return users[username].get("package", "basic")

def get_user_by_custom_url(custom_url):
    users = get_vip_users()
    for username, data in users.items():
        if data.get("custom_url", "").lower() == custom_url.lower():
            return username, data
    return None, None

def increment_vip_api_call(username):
    users = get_vip_users()
    if username in users:
        users[username]["total_api_calls"] = users[username].get("total_api_calls", 0) + 1
        users[username]["daily_api_calls"] = users[username].get("daily_api_calls", 0) + 1
        users[username]["last_api_call"] = datetime.datetime.utcnow().isoformat()
        save_vip_users(users)
        return True
    return False

# ============================================
# VIP SESSION DECORATORS
# ============================================

def vip_session_required(f):
    @wraps(f)
    def decorated(*args, **kwargs):
        if session.get("vip_authenticated"):
            username = session.get("vip_username")
            if is_user_expired(username):
                session.clear()
                flash("⏰ Your VIP subscription has expired!", "error")
                return redirect(url_for('vip_login_page'))
...
def admin_session_required(f):
    @wraps(f)
    def decorated(*args, **kwargs):
        if session.get("vip_authenticated") and session.get("role") == "admin":
            return f(*args, **kwargs)
        flash("⚠️ Admin access required!", "error")
        return redirect(url_for('vip_login_page'))
    return decorated

# ============================================
# ORIGINAL USER FUNCTIONS (UNCHANGED)
# ============================================
...
def get_user_role_old(username):
    users = get_users()
    if username not in users:
        return None
    return users[username].get("role", "user")
...
# ============================================
# ORIGINAL USER DASHBOARD (UNCHANGED)
# ============================================
USER_PANEL_HTML = '''
<!DOCTYPE html>
<html lang="en">
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="action-row">
                    ...
                    {% endif %}
                    <a href="{{ url_for('vip_login_page') }}" class="admin" style="border-color:rgba(255,215,0,0.1); color:#ffd700;"><i class="fas fa-crown"></i> VIP Login</a>
...
# ============================================
# ORIGINAL ADMIN LOGIN PAGE (UNCHANGED)
# ============================================
ADMIN_LOGIN_HTML = '''
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .bg-animation .orb:nth-child(1) { width: 400px; height: 400px; background: rgba(255,215,0,0.03); top: -100px; left: -100px; animation-delay: 0s; }
        .bg-animation .orb:nth-child(2) { width: 500px; height: 500px; background: rgba(255,215,0,0.02); bottom: -150px; right: -150px; animation-delay: -5s; }
        .bg-animation .orb:nth-child(3) { width: 300px; height: 300px; background: rgba(255,215,0,0.015); top: 50%; left: 50%; transform: translate(-50%, -50%); animation-delay: -10s; }
        ...
        .login-container { position: relative; z-index: 1; background: rgba(6,6,12,0.92); border: 1px solid rgba(255,215,0,0.15); border-radius: 24px; padding: 48px 40px; max-width: 420px; width: 92%; backdrop-filter: blur(40px); box-shadow: 0 0 80px rgba(255,215,0,0.05); animation: containerIn 0.8s cubic-bezier(0.16,1,0.3,1); }
        ...
        .brand-section .icon-wrap { display: inline-block; font-size: 32px; color: #ffd700; opacity: 0.5; margin-bottom: 4px; }
        .brand-section h1 { font-family: 'Orbitron', monospace; font-size: 22px; font-weight: 800; letter-spacing: 4px; color: #fff; }
        .brand-section h1 .highlight { color: #ffd700; text-shadow: 0 0 40px rgba(255,215,0,0.2); }
        .brand-section .tagline { font-size: 9px; font-weight: 400; letter-spacing: 6px; color: #88ddff; text-transform: uppercase; margin-top: 2px; font-family: 'Orbitron', monospace; }
        .brand-section .divider { width: 40px; height: 1px; background: rgba(255,215,0,0.3); margin: 10px auto 0; }
        .form-group { margin-bottom: 16px; }
        ...
        .input-wrap { display: flex; align-items: center; background: rgba(0,0,0,0.3); border: 1px solid rgba(255,215,0,0.15); border-radius: 12px; transition: all 0.3s ease; overflow: hidden; }
        .input-wrap:focus-within { border-color: rgba(255,215,0,0.6); box-shadow: 0 0 30px rgba(255,215,0,0.05); }
        .input-wrap .prefix { padding: 12px 0 12px 16px; color: #88ddff; font-size: 13px; width: 38px; text-align: center; font-family: 'Orbitron', monospace; }
        .input-wrap .line { width: 1px; height: 20px; background: rgba(255,215,0,0.1); }
        .input-wrap input { flex: 1; padding: 12px 16px; background: transparent; border: none; color: #fff; font-size: 15px; outline: none; font-family: 'Inter', sans-serif; }
        .input-wrap input::placeholder { color: #88ddff; font-size: 13px; }
        .btn-login { width: 100%; padding: 14px; background: rgba(255,215,0,0.05); border: 1px solid rgba(255,215,0,0.2); border-radius: 12px; color: #88ddff; font-size: 13px; font-weight: 600; cursor: pointer; font-family: 'Orbitron', monospace; letter-spacing: 4px; transition: all 0.3s ease; text-transform: uppercase; display: flex; justify-content: center; align-items: center; gap: 12px; margin-top: 4px; }
        .btn-login:hover { border-color: rgba(255,215,0,0.5); color: #ffd700; box-shadow: 0 0 40px rgba(255,215,0,0.05); }
        .btn-login i { font-size: 14px; color: #ffd700; }
        .error-text { color: #ff3355; font-size: 11px; padding: 6px 0; display: none; text-align: center; font-family: 'Orbitron', monospace; }
        .error-text.show { display: block; animation: shake 0.4s ease; }
        @keyframes shake { 0%,100%{transform:translateX(0)} 25%{transform:translateX(-4px)} 75%{transform:translateX(4px)} }
        .status-bar { display: flex; justify-content: center; gap: 20px; margin-top: 18px; padding: 10px 14px; background: rgba(0,0,0,0.15); border-radius: 10px; border: 1px solid rgba(255,215,0,0.05); flex-wrap: wrap; }
        ...
        .notice-box { margin-top: 14px; padding: 8px 12px; background: rgba(255,215,0,0.03); border: 1px solid rgba(255,215,0,0.1); border-radius: 8px; text-align: center; font-size: 7px; font-family: 'Orbitron', monospace; color: #88ddff; line-height: 1.6; }
        .notice-box .gold { color: #ffd700; }
        @media (max-width: 480px) { .login-container { padding: 32px 22px; } .brand-section h1 { font-size: 18px; } .status-bar { gap: 12px; flex-wrap: wrap; } }
    </style>
</head>
<body>
    <div class="bg-animation"><div class="orb"></div><div class="orb"></div><div class="orb"></div></div>
    <div class="login-container">
        <div class="brand-section">
            ...
            <h1><span class="highlight">VIP</span> ACCESS</h1>
            <div class="tagline">premium · system · elite</div>
            <div class="divider"></div>
        </div>
        <form method="POST">
            <div class="form-group">
                ...
                <div class="input-wrap">
                    <div class="prefix"><i class="fas fa-user"></i></div>
                    <div class="line"></div>
                    <input type="text" name="username" placeholder="Enter VIP username" required autofocus>
                </div>
            </div>
            <div class="form-group">
                <label><i class="fas fa-key"></i> password</label>
                <div class="input-wrap">
                    <div class="prefix"><i class="fas fa-lock"></i></div>
                    <div class="line"></div>
                    <input type="password" name="password" placeholder="Enter VIP password" required>
                </div>
            </div>
            <div class="error-text" id="loginError">{{ error }}</div>
            <button type="submit" class="btn-login"><i class="fas fa-unlock-alt"></i> unlock vip</button>
        </form>
        <div class="status-bar">
            <span class="item"><i class="fas fa-database"></i> secure</span>
            <span class="item"><i class="fas fa-clock"></i> 24/7</span>
            <span class="item"><i class="fas fa-shield-alt"></i> encrypted</span>
        </div>
        <div class="notice-box">
            <span class="gold">👑</span> Buy VIP subscription to access premium features<br>
            <span style="color:#ffd700;">Contact: +919242428894</span>
        </div>
        <div class="footer-text">⚡ sakil bhai · vip premium system ⚡</div>
    </div>
    <script>
        document.querySelector('input[name="username"]').focus();
        document.querySelectorAll('input').forEach(el => {
...
# ============================================
# VIP DASHBOARD (NEW - USES ORIGINAL PANEL WITH CUSTOM URL)
# ============================================
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">
    <title>{{ custom_domain|upper }} · VIP SYSTEM</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">
    <link href="https://fonts.googleapis.com/css2?family=Orbitron:wght@400;500;600;700;800;900&family=Inter:wght@300;400;500;600;700;800&display=swap" rel="stylesheet">
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        * { -webkit-user-select: none; -moz-user-select: none; -ms-user-select: none; user-select: none; }
        ...
        .bg-animation .orb:nth-child(1) { width: 500px; height: 500px; background: rgba(255,215,0,0.015); top: -150px; left: -150px; }
        .bg-animation .orb:nth-child(2) { width: 600px; height: 600px; background: rgba(255,215,0,0.01); bottom: -200px; right: -200px; animation-delay: -8s; }
        .bg-animation .orb:nth-child(3) { width: 300px; height: 300px; background: rgba(255,215,0,0.008); top: 50%; left: 50%; transform: translate(-50%, -50%); animation-delay: -15s; }
        ...
        .session-bar .fill { height: 100%; background: linear-gradient(90deg, #ffd700, #ff6600); width: 100%; transition: width 1s linear; box-shadow: 0 0 30px rgba(255,215,0,0.02); }
        .session-bar .fill.warning { background: linear-gradient(90deg, #FF9933, #ff3355); }
        .tricolor { width: 100%; height: 3px; display: flex; position: fixed; top: 2px; left: 0; z-index: 999; }
        .tricolor .saffron { width: 33.33%; background: #FF9933; }
        .tricolor .white { width: 33.33%; background: #FFFFFF; }
        .tricolor .green { width: 33.34%; background: #138808; }
        .top-bar { width: 100%; background: rgba(0,0,0,0.8); padding: 6px 0; margin-top: 5px; position: sticky; top: 5px; z-index: 100; border-bottom: 1px solid rgba(255,215,0,0.1); backdrop-filter: blur(20px); }
        ...
        .top-bar .left i { color: #ffd700; margin-right: 4px; }
        .top-bar .right { font-size: 8px; font-family: 'Orbitron', monospace; color: #88ddff; letter-spacing: 1px; }
        .top-bar .right i { color: #ffd700; margin-right: 4px; }
        .main-header { width: 100%; background: rgba(0,0,0,0.85); padding: 12px 0; border-bottom: 1px solid rgba(255,215,0,0.1); backdrop-filter: blur(20px); }
        ...
        .main-header .brand .icon { width: 40px; height: 40px; border-radius: 50%; border: 1px solid rgba(255,215,0,0.2); display: flex; align-items: center; justify-content: center; background: rgba(255,215,0,0.05); font-size: 18px; color: #ffd700; }
        .main-header .brand .text h1 { font-family: 'Orbitron', monospace; font-size: 16px; font-weight: 700; color: #fff; letter-spacing: 2px; }
        .main-header .brand .text h1 .hl { color: #ffd700; }
        ...
        .main-header .brand .text .sub i { color: #ffd700; font-size: 5px; margin-right: 3px; }
        .main-header .status { display: flex; align-items: center; gap: 10px; background: rgba(0,0,0,0.2); padding: 4px 14px; border-radius: 30px; border: 1px solid rgba(255,215,0,0.1); }
        ...
        .main-header .status .timer { font-size: 11px; font-weight: 600; color: #ffd700; font-family: 'Orbitron', monospace; padding: 1px 8px; border: 1px solid rgba(255,215,0,0.15); border-radius: 4px; background: rgba(255,215,0,0.05); }
        ...
        .main-header .status .role.admin { border-color: rgba(255,215,0,0.2); color: #ffd700; }
        .main-container { max-width: 560px; width: 100%; padding: 18px 14px; margin: 16px 0 30px; position: relative; z-index: 1; }
        .card { background: rgba(6,6,12,0.94); border: 1px solid rgba(255,215,0,0.1); border-radius: 20px; overflow: hidden; backdrop-filter: blur(30px); box-shadow: 0 8px 60px rgba(0,0,0,0.6); }
        .card-header { padding: 18px 22px; text-align: center; border-bottom: 1px solid rgba(255,215,0,0.05); background: rgba(0,0,0,0.2); }
        .card-header .badge { font-size: 7px; font-family: 'Orbitron', monospace; color: #88ddff; letter-spacing: 4px; text-transform: uppercase; border: 1px solid rgba(255,215,0,0.1); padding: 2px 14px; border-radius: 30px; display: inline-block; margin-bottom: 6px; }
        .card-header .badge i { color: #ffd700; font-size: 6px; margin-right: 4px; }
        .card-header .domain-badge { font-size: 12px; font-family: 'Orbitron', monospace; color: #ffd700; letter-spacing: 2px; background: rgba(255,215,0,0.05); border: 1px solid rgba(255,215,0,0.1); padding: 4px 14px; border-radius: 30px; display: inline-block; margin-bottom: 8px; }
        .card-header .domain-badge i { color: #ffd700; margin-right: 6px; }
        .card-header .icon { width: 48px; height: 48px; border-radius: 50%; border: 1px solid rgba(255,215,0,0.1); display: flex; align-items: center; justify-content: center; margin: 0 auto 6px; background: rgba(255,215,0,0.05); font-size: 22px; color: #ffd700; }
        .card-header h2 { font-family: 'Orbitron', monospace; font-size: 18px; font-weight: 700; color: #fff; letter-spacing: 2px; }
        .card-header h2 .hl { color: #ffd700; }
        .card-header .sub { font-size: 8px; font-family: 'Orbitron', monospace; color: #88ddff; letter-spacing: 4px; margin-top: 2px; }
        .card-header .sub i { color: #ffd700; font-size: 6px; margin-right: 4px; }
        .card-body { padding: 20px 22px; }
        .form-group { margin-bottom: 14px; }
        ...
        .form-group label i { color: #ffd700; margin-right: 4px; font-size: 8px; }
        .input-wrap { display: flex; align-items: center; background: rgba(0,0,0,0.3); border: 1px solid rgba(255,215,0,0.1); border-radius: 12px; transition: all 0.3s ease; overflow: hidden; }
        .input-wrap:focus-within { border-color: rgba(255,215,0,0.4); }
        .input-wrap .code { padding: 10px 6px 10px 14px; color: #88ddff; font-size: 12px; font-family: 'Orbitron', monospace; border-right: 1px solid rgba(255,215,0,0.05); }
        ...
        .status-line .dot.err { background: #ff3355; }
        .btn-execute { width: 100%; padding: 13px; background: rgba(255,215,0,0.05); border: 1px solid rgba(255,215,0,0.2); border-radius: 12px; color: #88ddff; font-size: 12px; font-weight: 600; cursor: pointer; font-family: 'Orbitron', monospace; letter-spacing: 4px; text-transform: uppercase; transition: all 0.3s ease; display: flex; justify-content: center; align-items: center; gap: 12px; }
        .btn-execute:hover { border-color: rgba(255,215,0,0.5); color: #ffd700; }
        .btn-execute:disabled { opacity: 0.2; cursor: not-allowed; }
        .btn-execute i { font-size: 13px; color: #ffd700; }
        .error-text { color: #ff3355; font-size: 10px; padding: 4px 0; display: none; font-family: 'Orbitron', monospace; letter-spacing: 1px; }
        .error-text.show { display: block; animation: shake 0.4s ease; }
        @keyframes shake { 0%,100%{transform:translateX(0)} 25%{transform:translateX(-4px)} 75%{transform:translateX(4px)} }
        .result-box { margin-top: 16px; border: 1px solid rgba(255,215,0,0.05); border-radius: 12px; overflow: hidden; display: none; background: rgba(0,0,0,0.1); }
        .result-box.show { display: block; animation: slideUp 0.4s ease; }
        @keyframes slideUp { 0%{opacity:0;transform:translateY(10px)} 100%{opacity:1;transform:translateY(0)} }
        .result-header { padding: 8px 16px; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid rgba(255,215,0,0.03); background: rgba(0,0,0,0.1); }
        .result-header .title { font-size: 8px; font-family: 'Orbitron', monospace; color: #88ddff; letter-spacing: 2px; }
        .result-header .title i { color: #ffd700; margin-right: 4px; }
        ...
        .result-item .label i { font-size: 10px; color: #ffd700; width: 14px; }
        ...
        .result-item .value.hl { color: #ffd700; font-weight: 500; }
        .result-item .value.gr { color: #00ff66; font-weight: 500; }
        .result-item .value.addr { font-size: 10px; color: #88ddff; line-height: 1.4; }
        .location-section { margin-top: 14px; border: 3px solid #ffd700; border-radius: 14px; overflow: hidden; display: none; background: rgba(0,0,0,0.1); box-shadow: 0 0 40px rgba(255,215,0,0.08); transition: all 0.4s ease; }
        .location-section.show { display: block; animation: slideUp 0.5s ease; }
        .location-section.live { border-color: #ff0000 !important; box-shadow: 0 0 50px rgba(255,0,0,0.15) !important; }
        .location-section.area { border-color: #ff0000 !important; box-shadow: 0 0 40px rgba(255,0,0,0.08) !important; }
        ...
        .location-section .map-container .location-badge { position: absolute; top: 12px; right: 12px; background: rgba(0,0,0,0.75); backdrop-filter: blur(10px); padding: 4px 14px; border-radius: 20px; font-size: 8px; font-family: 'Orbitron', monospace; color: #ffd700; border: 1px solid rgba(255,215,0,0.2); z-index: 10; letter-spacing: 1px; display: flex; align-items: center; gap: 6px; }
        ...
        .location-section .map-label { padding: 8px 14px; font-size: 7px; font-family: 'Orbitron', monospace; color: #88ddff; letter-spacing: 2px; text-align: center; background: rgba(0,0,0,0.15); border-top: 1px solid rgba(255,215,0,0.03); display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 4px; }
        .location-section .map-label .hint { color: #88ddff; }
        .location-section .map-label .hint i { color: #ff0000; margin-right: 4px; }
        .location-section .map-label .open-link { font-size: 7px; font-family: 'Orbitron', monospace; color: #ffd700; text-decoration: none; padding: 2px 12px; border: 1px solid rgba(255,215,0,0.1); border-radius: 20px; transition: all 0.3s ease; }
        .location-section .map-label .open-link:hover { background: rgba(255,215,0,0.05); border-color: rgba(255,215,0,0.3); }
        .badge-row { display: flex; justify-content: center; gap: 16px; padding: 10px 14px; margin-top: 12px; border: 1px solid rgba(255,215,0,0.03); border-radius: 10px; background: rgba(0,0,0,0.05); flex-wrap: wrap; }
        ...
        .badge-row .item i { font-size: 9px; color: #ffd700; }
        .social-row { margin-top: 12px; padding: 12px 16px; border: 1px solid rgba(255,215,0,0.05); border-radius: 10px; text-align: center; background: rgba(0,0,0,0.03); }
        ...
        .social-row .btns a:hover { border-color: rgba(255,215,0,0.2); color: #ffd700; }
        .social-row .btns a i { margin-right: 4px; }
        ...
        .json-toggle:hover { border-color: rgba(255,215,0,0.2); color: #ffd700; }
        .json-box { margin-top: 8px; background: rgba(0,0,0,0.2); border-radius: 8px; padding: 10px; font-family: 'Courier New', monospace; font-size: 8px; color: #88ddff; display: none; max-height: 150px; overflow: auto; white-space: pre-wrap; word-break: break-all; line-height: 1.5; border: 1px solid rgba(255,215,0,0.05); }
        .json-box.show { display: block; }
        ...
        .action-row a:hover { border-color: rgba(255,215,0,0.2); color: #ffd700; }
        .action-row a.admin { border-color: rgba(255,215,0,0.1); color: #ffd700; }
        .action-row a.admin:hover { border-color: rgba(255,215,0,0.3); color: #ffd700; }
        .action-row a.logout { border-color: rgba(255,51,85,0.1); color: #ff3355; }
        .action-row a.logout:hover { border-color: rgba(255,51,85,0.3); color: #ff3355; }
        .footer { width: 100%; background: rgba(0,0,0,0.85); border-top: 1px solid rgba(255,215,0,0.05); padding: 14px 0; margin-top: 10px; backdrop-filter: blur(20px); }
        ...
        .domain-url { color: #ffd700; font-family: 'Orbitron', monospace; font-size: 10px; letter-spacing: 1px; background: rgba(255,215,0,0.05); padding: 4px 12px; border-radius: 20px; border: 1px solid rgba(255,215,0,0.1); display: inline-block; }
        .package-badge { font-size: 7px; font-family: 'Orbitron', monospace; padding: 2px 12px; border-radius: 30px; border: 1px solid rgba(255,215,0,0.2); color: #ffd700; background: rgba(255,215,0,0.05); display: inline-block; }
        @media (max-width: 480px) { .main-header .brand .text h1 { font-size: 13px; } .main-header .brand .icon { width: 32px; height: 32px; font-size: 14px; } .main-header .status { padding: 3px 10px; } .main-header .status .timer { font-size: 9px; } .card-header h2 { font-size: 15px; } .card-body { padding: 14px 16px; } .result-item { flex-wrap: wrap; padding: 6px 12px; } .result-item .label { width: 100%; margin-bottom: 2px; } .result-item .value { width: 100%; text-align: left; } .main-container { padding: 12px 10px; } .badge-row { gap: 10px; } .location-section .map-container .location-badge { top: 8px; right: 8px; font-size: 6px; padding: 2px 10px; } }
    </style>
</head>
<body>
    <div class="session-bar"><div class="fill" id="sessionFill"></div></div>
    ...
    <div class="top-bar">
        <div class="container">
            <div class="left"><i class="fas fa-crown"></i> VIP · {{ custom_domain|upper }} SYSTEM</div>
            <div class="right"><i class="fas fa-circle" style="color:#00ff66;font-size:5px;"></i> active</div>
        </div>
    </div>
    <div class="main-header">
        <div class="container">
            <div class="brand">
                <div class="icon"><i class="fas fa-crown"></i></div>
                <div class="text">
                    <h1><span class="hl">{{ custom_domain|upper }}</span> VIP</h1>
                    <div class="sub"><i class="fas fa-circle"></i> premium · system</div>
                </div>
            </div>
            <div class="status">
                <div class="dot"></div>
                ...
                <span class="timer" id="sessionTimer">--:--</span>
                <span class="role {{ session.get('role', 'user') }}">{{ session.get('role', 'user')|upper }}</span>
                <a href="{{ url_for('vip_logout') }}" style="color:#88ddff; font-size:10px; text-decoration:none;">
                    <i class="fas fa-sign-out-alt"></i>
                </a>
            </div>
        </div>
    ...
    <div class="main-container">
        <div class="card">
            <div class="card-header">
                <div class="badge"><i class="fas fa-crown"></i> premium · vip</div>
                <div class="domain-badge">
                    <i class="fas fa-globe"></i> {{ custom_domain }}.{{ base_domain }}
                </div>
                <div class="icon"><i class="fas fa-crown"></i></div>
                <h2><span class="hl">{{ custom_domain|upper }}</span> DASHBOARD</h2>
                <div class="sub"><i class="fas fa-circle"></i> premium intelligence system</div>
            </div>
            <div class="card-body">
                <form id="trackForm">
                ...
                <div id="apiInfo" style="display:none; margin-top:10px; padding:8px 12px; background:rgba(0,0,0,0.05); border-radius:8px; display:flex; justify-content:center; gap:16px; flex-wrap:wrap; font-size:7px; color:#88ddff; border:1px solid rgba(255,255,255,0.03);"></div>
                <div class="notice">
                    <span class="g">👑</span> VIP System v9.0 <span class="g">👑</span><br>
                    <span style="color:#ffd700;">Your domain: {{ custom_domain }}.{{ base_domain }}</span>
                    <br><span class="package-badge">{{ package|upper }}</span>
                </div>
                <div class="action-row">
                    {% if session.get('role') == 'admin' %}
                    <a href="{{ url_for('vip_admin_panel') }}" class="admin"><i class="fas fa-crown"></i> admin panel</a>
                    {% endif %}
                    <a href="{{ url_for('vip_logout') }}" class="logout"><i class="fas fa-sign-out-alt"></i> logout</a>
                </div>
            </div>
        </div>
    </div>
    <div class="footer">
        <div class="container">
            ...
            <div class="copy">⚡ 2026 <span class="b">{{ custom_domain|upper }}</span> · vip premium system ⚡</div>
            <div class="copy" style="font-size:5px; margin-top:4px; color:#ffd700; letter-spacing:2px;">
                <i class="fas fa-star"></i> powered by <strong>SAKIL BHAI</strong> <i class="fas fa-star"></i>
            </div>
            <div class="tricolor"><div class="saffron"></div><div class="white"></div><div class="green"></div></div>
        </div>
    </div>
    ...
    function updateTimer() {
        ...
        if (totalSeconds <= 0) {
            ...
            fillEl.style.width = '0%';
            fillEl.className = 'fill warning';
            window.location.href = '{{ url_for("vip_logout") }}';
            return;
        }
        timerEl.textContent = formatTime(totalSeconds);
        const maxSec = 3600;
    ...
    function parseAreaName(address) {
        if (!address || address === 'N/A' || address === 'Unknown' || address === '') return null;
    ...
    function updateLocationMap(address, lat, lng, type) {
        ...
        if (!areaName) areaName = address.substring(0, 35);
    ...
    async function callAPI(number) {
        try {
            const response = await fetch('/vip/api/lookup', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ number: number })
            });
    ...
    function displayResults(number, data) {
        ...
        if (data.result && data.result.length > 0) {
            ...
            html += `<div class="result-item location-item ${locationClass}">
                <span class="label"><i class="fas ${hasLatLng ? 'fa-satellite-dish' : 'fa-map-pin'}" style="color:#ff0000;"></i> location</span>
                <span class="value addr" id="addressValue">${address}</span>
                <span style="font-size:6px; font-family:'Orbitron',monospace; color:#ff0000; margin-left:8px; border:1px solid #ff000040; padding:1px 8px; border-radius:10px; background:#ff000010;">
                    ${hasLatLng ? '🔴 LIVE' : '📍 AREA'}
                </span>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        console.log('🔥 VIP SYSTEM READY - {{ custom_domain|upper }}');
    });
    </script>
</body>
</html>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>VIP ADMIN · SAKIL BHAI</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">
    <link href="https://fonts.googleapis.com/css2?family=Orbitron:wght@400;500;600;700;800;900&family=Inter:wght@300;400;500;600;700;800&display=swap" rel="stylesheet">
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header .actions a.logout { border-color: rgba(255,51,85,0.1); color: #ff3355; }
        .header .actions a.logout:hover { border-color: rgba(255,51,85,0.3); color: #ff3355; }
        .card { background: rgba(6,6,12,0.92); border: 1px solid rgba(255,215,0,0.03); border-radius: 14px; padding: 16px 20px; margin-bottom: 14px; backdrop-filter: blur(20px); }
        ...
        .stat-box { background: rgba(0,0,0,0.15); border: 1px solid rgba(255,215,0,0.03); border-radius: 10px; padding: 10px 12px; text-align: center; }
        .stat-box .num { font-family: 'Orbitron', monospace; font-size: 20px; font-weight: 700; color: #fff; }
        .stat-box .num.green { color: #00ff66; }
        .stat-box .num.cyan { color: #ffd700; }
        .stat-box .num.red { color: #ff3355; }
        ...
        .badge.admin { border-color: rgba(255,215,0,0.2); color: #ffd700; }
        .badge.user { border-color: rgba(255,215,0,0.1); color: #88ddff; }
        .badge.active { border-color: rgba(0,255,102,0.2); color: #00ff66; }
        ...
        .actions-cell a.del:hover { border-color: rgba(255,51,85,0.2); color: #ff3355; }
        .add-form { display: flex; gap: 8px; flex-wrap: wrap; align-items: center; }
        .add-form input, .add-form select { padding: 6px 12px; background: rgba(0,0,0,0.2); border: 1px solid rgba(255,215,0,0.05); border-radius: 8px; color: #fff; font-size: 12px; outline: none; font-family: 'Inter', sans-serif; flex: 1; min-width: 100px; }
        ...
        .empty { text-align: center; color: #88ddff; padding: 16px; font-size: 10px; font-family: 'Orbitron', monospace; letter-spacing: 2px; }
        .footer-text { text-align: center; font-size: 6px; color: #88ddff; letter-spacing: 3px; margin-top: 10px; font-family: 'Orbitron', monospace; }
        @media (max-width: 600px) { .header .title h1 { font-size: 15px; } .stats { grid-template-columns: repeat(2, 1fr); } .add-form { flex-direction: column; } .add-form input, .add-form select, .add-form .btn-add { width: 100%; } .card { padding: 12px 14px; } }
    </style>
</head>
<body>
    <div class="container">
        <div class="header">
            <div class="title">
                <i class="fas fa-crown"></i>
                <div><h1><span class="hl">VIP</span> ADMIN</h1><div class="sub">premium · user management</div></div>
            </div>
            <div class="actions">
                <a href="{{ url_for('vip_dashboard') }}"><i class="fas fa-arrow-left"></i> back</a>
                <a href="{{ url_for('vip_logout') }}" class="logout"><i class="fas fa-sign-out-alt"></i> logout</a>
            </div>
        </div>
        ...
        <div class="stats">
            <div class="stat-box"><div class="num green">{{ stats.total_users }}</div><div class="label">users</div></div>
            <div class="stat-box"><div class="num cyan">{{ stats.active_users }}</div><div class="label">active</div></div>
            <div class="stat-box"><div class="num red">{{ stats.expired_users }}</div><div class="label">expired</div></div>
            <div class="stat-box"><div class="num" style="color:#ffd700;">{{ stats.premium_users }}</div><div class="label">premium</div></div>
        </div>

        <div class="card">
            <div class="card-title"><i class="fas fa-user-plus"></i> create vip user</div>
            <form method="POST" action="{{ url_for('vip_admin_add_user') }}" class="add-form">
                <input type="text" name="username" placeholder="username" required>
                <input type="password" name="password" placeholder="password" required>
                <input type="text" name="custom_url" placeholder="custom url (e.g. sakil)">
                <select name="package">
                    <option value="basic">Basic (30 days)</option>
                    <option value="premium" selected>Premium (60 days)</option>
                    <option value="enterprise">Enterprise (90 days)</option>
                </select>
                <input type="datetime-local" name="expiry" id="expiryInput">
                <button type="submit" class="btn-add"><i class="fas fa-plus"></i> create</button>
            </form>
            <script>
                document.getElementById('expiryInput').value = new Date(Date.now() + 60*24*60*60*1000).toISOString().slice(0,16);
            </script>
        </div>
        ...
        <div class="card">
            ...
            <div class="table-wrap">
                {% if users %}
                <table>
                    <thead><tr><th>username</th><th>url</th><th>package</th><th>status</th><th>expiry</th><th>actions</th></tr></thead>
                    <tbody>
                        {% for username, data in users.items() %}
                        <tr>
                            <td style="color:{% if username == 'sakil2026' %}#ffd700{% else %}#fff{% endif %}; font-weight:500;">
                                {{ username }} {% if username == 'sakil2026' %}<span style="color:#ffd700; font-size:7px;">⭐</span>{% endif %}
                            </td>
                            <td style="font-size:8px; color:#ffd700;">{{ data.get('custom_url', username) }}</td>
                            <td><span class="badge" style="border-color:rgba(255,215,0,0.2); color:#ffd700;">{{ data.get('package', 'basic') }}</span></td>
                            <td>
                                {% if data.get('active', True) %}
                                    {% if not is_expired(username) %}
                            ...
                            <td style="font-size:8px; color:#88ddff;">{{ data.get('expiry', 'N/A')[:10] if data.get('expiry') else 'N/A' }}</td>
                            <td>
                                <div class="actions-cell">
                                    <a href="{{ url_for('vip_admin_edit_user', username=username) }}"><i class="fas fa-pen"></i></a>
                                    <a href="{{ url_for('vip_admin_toggle_user', username=username) }}"><i class="fas fa-{% if data.get('active', True) %}pause{% else %}play{% endif %}"></i></a>
                                    {% if username != 'sakil2026' %}
                                    <a href="{{ url_for('vip_admin_delete_user', username=username) }}" class="del" onclick="return confirm('delete {{ username }}?')"><i class="fas fa-trash"></i></a>
                                    {% endif %}
                                </div>
                            </td>
                        </tr>
        ...
        <div class="footer-text">⚡ sakil bhai · vip admin system ⚡</div>
    </div>
</body>
</html>
'''
...
@app.route('/vip/login', methods=['GET', 'POST'])
def vip_login_page():
    if session.get("vip_authenticated"):
    ...
    if request.method == 'POST':
        ...
        if verify_vip_user(username, password):
            session["vip_authenticated"] = True
            session["vip_username"] = username
            session["role"] = get_user_role(username)
            session["custom_domain"] = get_user_url(username)
            session["package"] = get_user_package(username)
            flash(f"Welcome {username}! 🎉", "success")
            return redirect(url_for('vip_dashboard'))
        else:
            return render_template_string(VIP_LOGIN_HTML, error="Invalid credentials or expired subscription!")

    return render_template_string(VIP_LOGIN_HTML, error="")

@app.route('/vip/dashboard')
@vip_session_required
def vip_dashboard():
    username = session.get("vip_username")
    remaining = get_remaining_seconds_for_user(username)
    settings = get_vip_settings()
    return render_template_string(VIP_DASHBOARD_HTML,
                                 remaining_seconds=remaining,
                                 custom_domain=session.get("custom_domain", username),
                                 base_domain=settings.get("main_domain", "localhost:5000"),
...
def vip_admin_panel():
    users = get_vip_users()
    total = len(users)
    active = sum(1 for u in users.values() if u.get("active", True) and not is_user_expired(u))
    expired = sum(1 for u in users.values() if u.get("active", True) and is_user_expired(u))
    premium = sum(1 for u in users.values() if u.get("package") == "premium")
    stats = {"total_users": total, "active_users": active, "expired_users": expired, "premium_users": premium}
    return render_template_string(VIP_ADMIN_HTML, users=users, stats=stats, is_expired=is_user_expired)

@app.route('/vip/admin/add-user', methods=['POST'])
@vip_session_required
@admin_session_required
def vip_admin_add_user():
    username = request.form.get('username', '').strip()
    password = request.form.get('password', '')
    custom_url = request.form.get('custom_url', '').strip() or username
    package = request.form.get('package', 'premium')
    expiry_str = request.form.get('expiry', '')

    if not username or not password:
        flash("Username and password required!", "error")
        return redirect(url_for('vip_admin_panel'))
    ...
    if not expiry_str:
        packages = get_vip_settings().get("packages", {})
        days = packages.get(package, {}).get("days", 60)
        expiry = datetime.datetime.utcnow() + datetime.timedelta(days=days)
        expiry_str = expiry.strftime('%Y-%m-%dT%H:%M:%S+00:00')
    ...
    if save_vip_users(users):
        flash(f"VIP user '{username}' created! URL: /{custom_url.lower()}", "success")
    else:
        flash("Error creating user!", "error")

    return redirect(url_for('vip_admin_panel'))

@app.route('/vip/admin/toggle/<username>')
@vip_session_required
@admin_session_required
def vip_admin_toggle_user(username):
    if username == "sakil2026":
...
@app.route('/vip/admin/delete/<username>')
@vip_session_required
@admin_session_required
def vip_admin_delete_user(username):
    if username == "sakil2026":
...
@app.route('/vip/admin/edit/<username>', methods=['GET', 'POST'])
@vip_session_required
@admin_session_required
def vip_admin_edit_user(username):
    if username == "sakil2026":
    ...
    if request.method == 'POST':
        ...
        new_package = request.form.get('package', 'premium')
        new_expiry = request.form.get('expiry', '')
        new_url = request.form.get('custom_url', '').strip() or username

        users[username]["package"] = new_package
        users[username]["custom_url"] = new_url.lower()
        if new_password:
    ...
    return f'''
    <!DOCTYPE html>
    <html><head><title>edit vip user</title>
    <style>
        *{{margin:0;padding:0;box-sizing:border-box;}}
        ...
        .info{{font-size:8px;color:#88ddff;text-align:center;margin-bottom:10px;}}
    </style>
    </head>
    <body>
    <div class="box">
        <h1><span class="hl">edit</span> vip user</h1>
        <div class="sub">premium · {username}</div>
        <div class="info">Current URL: /{users[username].get('custom_url', username)}</div>
        <form method="POST">
            <div class="form-group"><label><i class="fas fa-key"></i> new password (optional)</label><input type="password" name="password" placeholder="leave blank"></div>
            <div class="form-group"><label><i class="fas fa-globe"></i> custom url</label><input type="text" name="custom_url" value="{users[username].get('custom_url', username)}"></div>
            <div class="form-group"><label><i class="fas fa-box"></i> package</label>
                <select name="package">
                    <option value="basic" {"selected" if users[username].get("package")=="basic" else ""}>Basic (30 days)</option>
                    <option value="premium" {"selected" if users[username].get("package")=="premium" else ""}>Premium (60 days)</option>
                    <option value="enterprise" {"selected" if users[username].get("package")=="enterprise" else ""}>Enterprise (90 days)</option>
                </select>
            </div>
            <div class="form-group"><label><i class="fas fa-clock"></i> expiry</label><input type="datetime-local" name="expiry" value="{users[username].get('expiry', '')[:16]}"></div>
            <button type="submit" class="btn"><i class="fas fa-save"></i> update</button>
        </form>
        <a href="{url_for('vip_admin_panel')}" class="back"><i class="fas fa-arrow-left"></i> back</a>
    </div>
    </body>
    </html>
    '''
...
# ============================================
# VIP API ROUTES (USES ORIGINAL API)
# ============================================

@app.route('/vip/api/lookup', methods=['POST'])
@vip_session_required
def vip_lookup():
    try:
        ...
        # Increment API call for VIP user
        username = session.get("vip_username")
        increment_vip_api_call(username)
...
# ============================================
# CUSTOM URL ROUTE (NEW) - /username
# ============================================

@app.route('/<custom_url>')
def vip_custom_url(custom_url):
    username, data = get_user_by_custom_url(custom_url)
    if not username:
        return "User not found", 404
    
    if not data.get("active", True):
        return "User inactive", 403
    ...
    session["custom_domain"] = custom_url
    session["package"] = data.get("package", "basic")
...
def login_page():
    if request.method == 'POST':
        ...
        if verify_user(username, password):
            ...
            session["role"] = get_user_role_old(username)
...
def user_dashboard():
    ...
    return render_template_string(USER_PANEL_HTML, remaining_seconds=remaining, max_seconds=3600)
...
def admin_dashboard():
    ...
    stats = {"total_users": total, "active_users": active, "inactive_users": inactive, "expiry_status": expiry_status}

    return render_template_string(ADMIN_DASHBOARD_HTML, users=users, stats=stats, remaining=remaining, expiry_local=expiry_local)
...
if __name__ == '__main__':
    ...
    print("="*60)
    print("⚡ SAKIL BHAI - USER PANEL v9.0 VIP PREMIUM")
    print("🔥 CYBERPUNK EDITION · MULTI-USER VIP SYSTEM")
    print("👑 CUSTOM URL: http://localhost:{port}/username")
    print("="*60)
    print(f"✅ Main Panel:  http://0.0.0.0:{port}")
    print(f"✅ Login:       http://0.0.0.0:{port}/login")
    print(f"✅ Admin:       http://0.0.0.0:{port}/admin")
    print(f"✅ VIP Login:   http://0.0.0.0:{port}/vip/login")
    print(f"✅ VIP Admin:   http://0.0.0.0:{port}/vip/admin")
    print("="*60)
    print("🔑 Default Admin: sakil2026 / sakil2026")
    print("🔑 VIP Admin: sakil2026 / sakil2026")
    print("📁 Data: vip_users.json, vip_settings.json")
    print("="*60)

    app.run(host='0.0.0.0', port=port, debug=False, threaded=True)    "appId": "1:1052436591563:web:e267676720461e7f937c4e",
    "measurementId": "G-LPE7H20EY6"
}
